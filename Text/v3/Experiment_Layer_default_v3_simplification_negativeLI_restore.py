...
from Text.v3.Behaviour_Core_Modules import *
...
class LearningInhibition(Behaviour):
    ...
    def set_variables(self, neurons):
        ...
        self.avg_inh = self.parameter('avg_inh', 0.28)
        self.lmt = self.parameter('lmt', 0.0)
    ...
    def new_iteration(self, neurons):
        ...
        neurons.linh = np.clip((1-inhibition/self.avg_inh) * self.strength, -self.lmt, 1.0)#0.1


class CreateWeights(Behaviour):

    def set_variables(self, synapses):
        distribution = self.parameter('distribution', 'uniform(0.0,1.0)')
...
IP_s = gene('IP_s', 0.008644124429332259)
avg_inh = gene('avg_inh', 0.33379413762020216)
LI_s = gene('LI_s', 7.728711263682648)
STDP_s = gene('STDP_s', 0.0025713481006048714)
fe_exp = gene('fe_exp', 0.5803961466810694)
fe_mul = gene('fe_mul', 2.1850529679529758)
...
NeuronGroup(net=net, tag='exc_neurons1', size=getGrid(n_exc_neurons), color=blue, behaviour={
    ...
    # learning
    40: LearningInhibition(transmitter='GABA', strength=LI_s, avg_inh=avg_inh, lmt=gene('LMT', 0.1)),
