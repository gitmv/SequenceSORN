...
#print(net.simulate_iteration(measure_behavior_execution_time=True))
...
if __name__ == '__main__' and ui:
    ...
    net.exc_neurons.add_behavior(90, Recorder(variables=['np.mean(n.output)', 'n.iteration']))
    net['TextReconstructor', 0].reconstruction_history = ''
    ...
    for influence_c, influence_strength in [['#',None], ['#',None], ['#',None], ['f',1], ['f',0.5], ['f',0.1], ['b',1], ['b',0.5], ['b',0.1], ['p',1], ['p',0.5], ['p',0.1]]:
        ...
        for i in range(100):
            if influence_c != '#':
                if net['TextReconstructor', 0].reconstruction_history.endswith('. '):
                    influence_char_index = net['TextGenerator', 0].char_to_index(influence_c)
        ...
        plt.plot(net['n.iteration',0], net['np.mean(n.output)',0], label=net['TextReconstructor', 0].reconstruction_history)
