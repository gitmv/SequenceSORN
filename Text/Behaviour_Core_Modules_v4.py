    ...
    def new_iteration(self, neurons):
        for s in neurons.afferent_synapses[self.transmitter]:
            ...
            if neurons.LearningInhibition.min<0:
                ...
                #faster?
                #wc_smaller_zero = weight_change<0.0
                #s.W[np.ix_(s.dst.output, s.src.output_old)][wc_smaller_zero] = np.clip(s.W[np.ix_(s.dst.output, s.src.output_old)][wc_smaller_zero], 0.0, None)
