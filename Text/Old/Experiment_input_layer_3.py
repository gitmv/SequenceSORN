...
inh_output_slope = gene('I', 18.222202430254626)#16.8

#LI_threshold = np.tanh(inh_output_slope * target_activity)

LI_threshold = gene('L', 0.31)#0.2#0.25
...
NeuronGroup(net=net, tag='exc_neurons', size=get_squared_dim(neuron_count), color=blue, behaviour={#60 30#NeuronDimension(width=10, height=10, depth=1)
    ...
    12: Synapse_Operation(transmitter='EE', strength=1.0),
    13: Synapse_Operation(transmitter='ES', strength=1.0),
    ...
    41.1: STDP(tag='STDP_ES', transmitter='ES', strength=gene('S',0.0019)),#0.0015
...
if __name__ == '__main__' and ui:
    ...
    for i in range(20):
        train_ES_and_get_distribution_score(net, 5000, deactivateEE=False)
