...
def add_layer(net, layer, layer_size):
    ...
    NeuronGroup(net=net, tag='exc_neurons'+str(layer), size=getGrid(layer_size), color=e_color, behaviour={
        # normalization
        3: Normalization(direction='afferent and efferent', syn_type='DISTAL', exec_every_x_step=200),
    }|({3.1: Normalization(direction='afferent', syn_type='SOMA', exec_every_x_step=200)} if layer==1 else {})|{ #only add behaviour for first layer
    ...
    NeuronGroup(net=net, tag='inh_neurons'+str(layer), size=get_squared_dim(layer_size/10), color=i_color, behaviour={
        # excitatory input
    ...
    SynapseGroup(net=net, tag='GLU,DISTAL', src='exc_neurons' + str(layer), dst='exc_neurons' + str(layer), behaviour={
        1: CreateWeights(normalize=False)
    })

    SynapseGroup(net=net, tag='GLUI', src='exc_neurons' + str(layer), dst='inh_neurons' + str(layer), behaviour={
        1: CreateWeights()
    })

    SynapseGroup(net=net, tag='GABA', src='inh_neurons' + str(layer), dst='exc_neurons' + str(layer), behaviour={
        1: CreateWeights()
    })


def add_connection(net, src_nr, dst_nr):
    SynapseGroup(net=net, tag='GLU,DISTAL', src='exc_neurons' + str(src_nr), dst='exc_neurons' + str(dst_nr), behaviour={
...
#add input layer
#net.exc_neurons1.add_behaviour(3.1, Normalization(direction='afferent', syn_type='SOMA', exec_every_x_step=200), initialize=False)


    #SynapseGroup(net=net, tag='E'+previous_layer+'E'+layer+',GLU,DISTAL', src='exc_neurons'+layer, dst='exc_neurons'+previous_layer, behaviour={
    #    1: CreateWeights(normalize=False)
    #})


'''   
    NeuronGroup(net=net, tag='exc_neurons,exc_neurons1', size=getGrid(layer_sizes[0]), color=blue, behaviour={
        # normalization
    ...
    NeuronGroup(net=net, tag='inh_neurons,inh_neurons1', size=getGrid(layer_sizes[0]/10), color=red, behaviour={
        # excitatory input
        60: SynapseOperation(transmitter='GLUI', strength=1.0),
    ...
    SynapseGroup(net=net, tag='ES,GLU,SOMA', src='inp_neurons', dst='exc_neurons', behaviour={
        1: CreateWeights(nomr_fac=10)
    })

    SynapseGroup(net=net, tag='EE,GLU,DISTAL', src='exc_neurons', dst='exc_neurons', behaviour={
        1: CreateWeights(normalize=False)
    })
