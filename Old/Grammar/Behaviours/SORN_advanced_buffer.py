    ...
    def initialize(self, neurons):
        ...
        for s in neurons.afferent_synapses[self.transmitter]:
            ...
            if self.distribution is not None:
                s.W = s.get_synapse_mat(self.distribution, density=float(density*weighting))#density*weighting
            else:
...
class SORN_temporal_synapses(Behavior):
    ...
    def iteration(self, neurons):
        ...
        if last_cycle_step(neurons):
            for s in neurons.afferent_synapses[self.syn_type]:
                #s.W_temp += (s.get_synapse_mat()+0.15)*(s.get_synapse_mat('uniform')>0.99)
...
class SORN_generate_output(Behavior):#has to be executed AFTER intra, inter...behaviors
    ...
    def initialize(self, neurons):
        ...
        neurons.TH = neurons.get_neuron_vec()+self.get_init_attr('init_TH', 0.0, neurons)#neurons.get_neuron_vec('uniform')*0.5
...
class buffer_variables(Behavior):#has to be executed AFTER intra, inter...behaviors
    ...
    def initialize(self, neurons):
        ...
        if self.get_init_attr('random_temporal_output_shift', False) and neurons.timescale > 1:
