...
class SORN_init_afferent_synapses(Behavior):
    ...
    def initialize(self, neurons):
        ...
        if density_mode == 'full':
            ...
            #s.get_synapse_mat('uniform',density=syn_lamb)
        else:
        ...
        for s in neurons.afferent_synapses[self.transmitter]:
            ...
            if self.distribution is not None:
                s.W = s.get_random_synapse_mat(density=float(density*weighting), rnd_code=self.distribution)#density*weighting get_synapse_mat('uniform')#todo:replace
            else:
...
class SORN_init_neuron_vars(Behavior):
    ...
    def initialize(self, neurons):
        ...
        neurons.TH = neurons.get_neuron_vec()+self.get_init_attr('init_TH', 0.5, neurons)#neurons.get_neuron_vec('uniform')*0.5
...
class SORN_slow_input_collect(Behavior):#has to be executed AFTER intra, inter...behaviors
...
class SORN_input_collect(Behavior):#has to be executed AFTER intra, inter...behaviors
...
class SORN_Refractory(Behavior):
...
class SORN_random_act(Behavior):
    ...
    def iteration(self, neurons):
        if last_cycle_step(neurons):
            ...
            add=0.1*(neurons.get_neuron_vec('uniform')<c)
...
'''
class SORN_init_variables(Neuron_Behavior):
    ...
    def initialize(self, neurons):
        ...
        for s in neurons.afferent_synapses['GLU']:
            ...
            if neurons.FSC:
                s.W = s.get_synapse_mat('uniform',density=syn_lamb)*s.enabled
            else:
                s.W = s.get_synapse_mat('uniform',density=syn_lamb / float(neurons.size))*s.enabled
            s.enabled *= (s.W > 0)
