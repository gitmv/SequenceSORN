...
exc_neurons = NeuronGroup(net=SORN, tag='exc_neurons', size=get_squared_dim(neuron_count), color=blue, behavior={
    ...
    16: TextActivator(input_density='[0.04#inpd]', strength='1.0'),#1.0 #0.75 #[0.9#IS]
    18: SynapseOperation(transmitter='GLU', strength='1.0'),

    #interneuron
    #23: Collect_Synapse_Input(transmitter='GABA'),
    #24: Apply_Synapse_Input(transmitter='GABA', strength=-1),
    24: SynapseOperation(transmitter='GABA', strength=-1),#[4.75#S]#'-0.72'

    # stability
    21: IP(sliding_window='0', speed='[0.007#IP]'),

    #learning
    25: LearningInhibition_mean2(strength='[31#LIM]', threshold='[0.377#th]', inh_attr='input_GABA'),  # 170#
    26: STDP_C(transmitter='GLU', eta_stdp='[0.0015#STDP]', STDP_F={-1: 1}),
    27: Normalization(syn_type='GLU', exec_every_x_step='10'),
    28: Out_Normalization(syn_type='GLU', exec_every_x_step='10'),
...
SynapseGroup(net=SORN, src=exc_neurons, dst=exc_neurons, tag='EE,GLU', behavior={
    #init
...
inh_neurons = NeuronGroup(net=SORN, tag='inh_neurons', size=get_squared_dim(neuron_count/10), color=red, behavior={
    2: Init_Neurons(),
    45: SynapseOperation(transmitter='GLUI', strength=1),
    46: inh_sigmoid_response(slope='20', duration=2),#[29.4#E]#strength='1', duration='[2#D]', 7
})

SynapseGroup(net=SORN, src=exc_neurons, dst=inh_neurons, tag='IE,GLUI', behavior={
    3: create_weights(distribution='uniform(1.0,1.0)', density=1.0, normalize=True)
})

SynapseGroup(net=SORN, src=inh_neurons, dst=exc_neurons, tag='EI,GABA', behavior={
    3: create_weights(distribution='uniform(1.0,1.0)', density=1.0, normalize=True)
})
...
#set_genome({'S': 4.7464756458591575, 'D': 2.042081612301938, 'E': 29.391792454679823, 'exp': 0.6140642657418552})
    ...
    # 24: SynapseOperation(transmitter='GABA', strength='-[1.0#GABAE]'),
    # 30: ReLu_Output_Prob(),
