...
from PymoNNto.NetworkBehavior.Input.Music.MusicActivator import *
...
def run(tag, ind=[], par={'N_e':2000, 'TS':[1], 'ts_plastic':1000, 'ts_train':1000, 'ts_test':100}):
    ...
    for lag in par['TS']:#
        e_ng = NeuronGroup(net=SORN, tag='exc_cell_{},prediction_source,text_input_group'.format(lag), size=get_squared_dim(int(par['N_e'])), behavior={
        ...
        i_ng = NeuronGroup(net=SORN, tag='inh_cell_{}'.format(lag), size=get_squared_dim(int(0.2 * (par['N_e']))), behavior={
