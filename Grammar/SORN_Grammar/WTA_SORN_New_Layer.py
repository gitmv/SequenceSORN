...
from Grammar.SORN_Grammar.Behaviours_New.Synapse_Operation import *
...
input_neurons = NeuronGroup(net=SORN, tag='input_neurons', size=None, behaviour={#get_squared_dim(23)
    2: init_neuron_variables(),
    3: init_synapses_simple(transmitter='GLU'),
    5: Text_Generator(text_blocks=[' fox eats meat.', ' boy drinks juice.', ' penguin likes ice.'], set_network_size_to_alphabet_size=True),
    6: Text_Activator_Simple(),
    7: SORN_generate_output_K_WTA(K=1),

    15: buffer_variables(),#for STDP

    21: STDP_complex(transmitter='GLU', eta_stdp='[0.00015#STDP_eta]', STDP_F={-1: 0.2, 1: -1}),
    22: Normalization(syn_type='GLU', behaviour_norm_factor=1.0),

    40: Text_Reconstructor_Simple()
})

exc_neurons = NeuronGroup(net=SORN, tag='exc_neurons', size=get_squared_dim(N_e), behaviour={
    2: init_neuron_variables(),
    ...
    9: input_synapse_operations(strength='10.0'),

    10: synapse_operation(transmitter='GLU', strength='1.0'),
    11: IP_apply(),
    12: refrac_apply(strengthfactor='[0.1#rs]'),
    13: K_WTA_output_local(partition_size=7, K='[0.02#k]'),
    ...
    20: IP(h_ip='lognormal_real_mean([0.02#IP_mean], [0.2944#IP_sigma])', eta_ip='[0.007#IP_eta]'),
    21: STDP_complex(transmitter='GLU', eta_stdp='[0.00015#STDP_eta]', STDP_F={-1: 0.2, 1: -1}),
    22: Normalization(syn_type='GLU', behaviour_norm_factor=1.0),
...
SynapseGroup(net=SORN, src=input_neurons, dst=exc_neurons, tag='Input_GLU,syn', behaviour={})
...
if __name__ == '__main__' and ui:
    ...
    input_neurons.color = get_color(1, 1)
    exc_neurons.color = get_color(0, 1)
    my_modules = get_default_UI_modules(['output', 'activity'])+get_my_default_UI_modules()# , 'nox', 'refractory_counter' #, 'exhaustion_value', 'weight_norm_factor'
    ...
    Network_UI(SORN, modules=my_modules, label='SORN K_WTA Layer', storage_manager=sm, group_display_count=2, reduced_layout=False).show()
