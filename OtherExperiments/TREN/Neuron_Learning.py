    ...
    def set_variables(self, neurons):
        ...
        for s in neurons.afferent_synapses['GLU']:
            ...
            if self.set_weights:
                ...
                s.enabled *= s.get_synapse_mat('uniform',density=self.GLU_density, clone_along_first_axis=all_neurons_same) > 0.0
                new_syn = s.enabled * (s.get_synapse_mat('uniform',density=1.0, clone_along_first_axis=all_neurons_same) * self.GLU_random_factor + self.GLU_equal_factor)
                #print(new_syn)
            else:
